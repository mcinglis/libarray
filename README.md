...
There is no `build` command specified for Libarray, because you should manage the building of Libarray's sources in your own project. The dependencies rely on generated source files, and you would want to have that process integrated with the rest of your project, avoiding multiple libraries trying to generate the same file differently. Despite this, there is a `Makefile` provided with the bare minimum to build the tests.
